...
struct DashboardView: View {
    ...
    // Calculated percentages - only for total income (30-day comparison)
    private var totalIncomePercentageChange: Double {
        return jobAmountService.thirtyDayPercentageChange
    }
    
    // Helper function to format percentage with proper sign and color
    @ViewBuilder
    private func percentageView(for percentage: Double, showIfZero: Bool = false) -> some View {
        if percentage != 0 || showIfZero {
            let isIncrease = percentage > 0
            let symbol = isIncrease ? "+" : ""
            let color = isIncrease ? Color.green : Color.red
            
            Text("\(symbol)\(String(format: "%.1f", percentage))%")
                .font(.subheadline)
                .fontWeight(.medium)
    ...
    @State private var selectedTimePeriod = "Last 6 months"
    ...
    var body: some View {
        ...
        .actionSheet(isPresented: $showingTimePeriodPicker) {
            ActionSheet(
                ...
                buttons: [
                    .default(Text("Day")) { selectedTimePeriod = "Day" },
                    .default(Text("Week")) { selectedTimePeriod = "Week" },
                    .default(Text("Month")) { selectedTimePeriod = "Month" },
                    .default(Text("Last 6 months")) { selectedTimePeriod = "Last 6 months" },
                    .default(Text("Year")) { selectedTimePeriod = "Year" },
                    .cancel()
                ]
            )
        }
            ...
            VStack(spacing: 16) {
                HStack {
                    CountUpCurrency(value: .constant(netValue), duration: 0.8)
                        .font(.dashboardMediumNumber)
                        .foregroundColor(Color.appPrimaryText)
                    
                    Spacer()
                    
                    percentageView(for: totalIncomePercentageChange)
                }
        ...
        VStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack {
                    ...
                    CountUpPercentage(value: .constant(progressPercentage), duration: 1.0, animationDelay: 0.3)
                        .font(.subheadline)
                        .fontWeight(.medium)
                        .foregroundColor(.blue)
                        .padding(.horizontal, 8)
    ...
    private func saveMonthlyGoal(_ goal: Double) {
        ...
        // Recalculate metrics to update progress
        calculateDashboardMetrics()
    }
    ...
    private func calculateDashboardMetrics() {
        ...
        if monthlyGoal > 0 {
            progressPercentage = (currentMonthNetEarnings / monthlyGoal) * 100
            targetProgress = min(currentMonthNetEarnings / monthlyGoal, 1.0)
        } else {
    ...
    private func generatePaidIncomeData() -> [(period: String, earnings: Double)] {
        return jobAmountService.getChartData()
    }
    ...
    private func calculateDynamicYAxisMax() -> Double {
        return jobAmountService.calculateChartYAxisMax()
    }
